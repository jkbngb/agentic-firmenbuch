...
```
**Rules:** keep in-scope groups; reserved groups = `null`; copy `identity.status` from `99_registry` (so the MCP can filter active vs gelöscht); **management gating** — names stay withheld in v1 (gated behind `expose_personal_data`, default False), but **`age_at_signing`, current `age`, and `birth_year` (year only) ARE exposed** for the primary manager / signatories, alongside `n_signatories_latest` and `signatories_stable_years`. Never serve the name, month, or day. (Rationale: year-of-birth without a name, month, or day is a strong GDPR minimization and is the core succession signal the product needs; the name remains the identifying element and stays gated.) add public `provenance` (source, license, attribution, data_version, built_at) and **omit internal hash chain** from the served body; denormalize `identity.status`, `financials.latest.*`, `ratios.equity_ratio.latest`, `has_guv_latest`, etc. to indexed shallow paths.
